...
/* Si salva in cache SOLO quello che il server ha davvero consegnato.
   Prima veniva salvata qualsiasi risposta, 404 compresi: bastava che il
   gioco chiedesse un'immagine non ancora caricata perché quel "non
   esiste" restasse in memoria per sempre, e la foto non compariva più
   nemmeno dopo averla messa nella cartella. */
function salva(req, res){
  if(!res || !res.ok || res.type === 'opaque') return res;   // niente errori in cache
